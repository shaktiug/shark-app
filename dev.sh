...
az webapp config container set --docker-custom-image-name $IMAGE  --docker-registry-server-password $REGPAS --docker-registry-server-url $REGADD --docker-registry-server-user $DID --name $DEVAPP --resource-group $RS
